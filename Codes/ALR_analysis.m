function ALR_analysis( method, HP )
%cross validation work using ALR
...
for person = 0:14
    ...
    epsilon = 0.01;
    for j = 1:size(TestFeature,2)
        test = TestFeature(:,j);
        %%ALR
        cvx_begin quiet
            variable w(Training_number)
            minimize(norm(w,1))
            subject to
                norm(TrainingFeature * w - test, 2) <= epsilon;
        cvx_end
        
        Predict = TrainingGaze * w;
